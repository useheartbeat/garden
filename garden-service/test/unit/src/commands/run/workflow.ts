...
describe("RunWorkflowCommand", () => {
  ...
  it("should throw if attempting to write a file with a directory path that contains an existing file", async () => {
    ...
    await expectError(
      () => cmd.action({ ...defaultParams, args: { workflow: "workflow-a" } }),
      (err) =>
        expect(err.message).to.equal("Unable to write file 'garden.yml/foo.txt': EEXIST: file already exists, mkdir")
    )
  })
    ...
    await expectError(
      ...
      (err) =>
        expect(err.message).to.equal(
          "Unable to write file '.garden': EISDIR: illegal operation on a directory, open '/Users/edvald/Code/garden/garden-service/test/data/test-project-a/.garden'"
        )
    )
  })
})
